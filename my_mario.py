...
def driver():
    while(True):
        ...
        dragon_handler()
        fireball_handler()
        drawtext('Score : %s | Top score : %s' %(score, topscore), scorefont, window, 350, rect_cactus.bottom + 10)
        screen_updater()
...
def collision_handler(f):
    global topscore
    if(rect_mario.collidepoint(f.left+10,f.top+10) or rect_mario.collidepoint(f.left+10,f.bottom-10) or rect_cactus.collidepoint(rect_mario.right,rect_mario.top+10) or rect_fire.collidepoint(rect_mario.right,rect_mario.bottom-10)):
        if(score>topscore):
            topscore=score
...
def dragon_handler():
    global dirc
    if((rect_dragon.top>50 and dirc=='goingup') or (rect_dragon.bottom>=rect_fire.top)):
        dirc='goingup'
        rect_dragon.top-=3
